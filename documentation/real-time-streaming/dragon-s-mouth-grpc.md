...
Some cloud providers (e.g. Cloudflare) close idle streams. To avoid this, keep sending pings to the server; it replies to each with a `pong`. The server also sends its own `ping` update on the stream every \~10 seconds to detect dead clients. You can ignore those or use them as a liveness signal.
